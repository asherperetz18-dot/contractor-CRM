...
## 035 — Project net cash counts the rep's cut

**Date:** 2026-09-17

**Context:** The Projects page said a complete job "kept" collected minus spent — $9,114 on a $13,000 job whose salesperson was owed $3,582 of it. Spent counts vendor money only, so the one number the owner reads as "mine" quietly included payroll. The owner asked for rep commission to be included.

**Decision:** Commission joins the rollup, not the Spent figure: `computeProjectRollup` takes an optional `commissionCents` and subtracts it in `netCashCents`, so the rows, the cards, the bleeding chip, the triage order and the printable projects report all move together, while Spent keeps meaning vendor money (it feeds nothing back into the commission calculation, which reads job costs directly). The figure itself is `commissionOwedCents`: the seated shares only — an empty seat's slice of the pool is not a debt — computed in `project-data.ts` with exactly the commission page's arithmetic (costsForContract attribution, stamped rates with company-default fallbacks, the assigned-rep fallback for a never-saved seat), so the two pages can never disagree about one job. Null, shown as a dash and subtracting nothing, when the figure would be dishonest: an uncosted job (its "commission" would be a share of the whole contract) or a cancelled one (nobody is owed). The input is optional because the single-project report (`/projects/[id]/report`) has a client view — pay never prints for a customer — so that report keeps its "collected − spent" arithmetic, and says so in its own label.

**Consequence:** Net cash is finally the ownable number, and jobs whose commission eats the margin surface in the bleeding triage. The trades: the per-job report's net cash now reads higher than the list's for the same job (each labels its own arithmetic; TECH_DEBT), and Profit & Loss still excludes commission.
