...
**Correction, 2026-09-10 (see #012): the nonce gap described above as "just `/login`/`/forgot-password`" was verified only against a local `next start` and turned out to be wrong for real production — it's every public page, for an entirely different and more serious reason. Don't rely on the "two known pages" framing above; read #012 before touching the enforcing/Report-Only split further.**

---

## 012 — The CSP nonce gap isn't two static pages, it's every page: Turbopack + Vercel's standalone output drops nonce injection entirely
**Date:** 2026-09-10
...
Root cause, found and confirmed, not guessed: [vercel/next.js#96063](https://github.com/vercel/next.js/issues/96063) — "Nonce-based CSP: no script gets a nonce on Turbopack + `output: 'standalone'`," closed upstream as **not planned**. This repo builds with Turbopack (the default builder in this Next.js version, unrelated to this app's own choices), and Vercel's own production build pipeline packages Next apps as `output: 'standalone'` regardless of what this repo's `next.config.ts` says. A local `next start` never goes through that same standalone packaging step, which is exactly why local verification in #011 didn't catch this — the discrepancy tracks the deployment target, not anything this app's code does differently per page.

**Decision:** No code change in response to this yet — logging it accurately is the priority so the next decision (see below) gets made deliberately, not by someone reading #011's now-incomplete "two pages" framing and concluding the checklist is nearly done. `e2e/public-smoke.spec.ts` was corrected to stop asserting "zero script-src violations on dynamic pages" (true locally, false in production) and instead only hard-asserts on *non*-script-src violations (connect-src/img-src/font-src — genuinely this app's own domain-allowlist correctness, unaffected by the Turbopack bug) while reporting script-src-elem counts as information rather than a target either direction.

**Consequence:** The CSP must **not** be flipped from Report-Only to enforcing under any of the current conditions — doing so today would break script execution, and therefore hydration, on every single production page, not a two-page edge case. Before that checklist in #011 can mean anything, someone needs to make one of these calls deliberately: (a) check whether building without Turbopack (webpack) avoids the upstream bug, and weigh switching the production build against whatever Turbopack was chosen for; (b) accept `'unsafe-inline'` in `script-src` as an explicit, understood fallback — CSP3 ignores `'unsafe-inline'` when a nonce is present and *working*, but with nonce injection broken as described here, `'unsafe-inline'` would actually be the only thing making scripts run at all if this were ever enforced, which means the nonce mechanism is providing no real protection against script injection in production today, Report-Only or not; or (c) wait for an upstream fix and re-run `e2e/public-smoke.spec.ts` against production periodically to check whether it's landed. Whoever makes that call should update this entry with the decision and the date.
